...
//Rare exceptions to indicate a bug in the _findatnd code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_findatnd = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_findatnd]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.findatnd.module"
